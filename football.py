...
@loader.tds
class FootballMod(loader.Module):
    """Football"""
    strings = {"name": "Football",
               "football_emoji_cfg_doc": "The emoji to be sent to Telegram as a football's. Can currently be either 🎲 or ⚽"}

    def __init__(self):
        self.config = loader.ModuleConfig("DICE_EMOJI", "'\U0001f3af'", lambda: self.strings["football_emoji_cfg_doc"])
    ...
    async def footballcmd(self, message):
        ...
        while True:
            task = client.send_message(chat, file=InputMediaDice(self.config["Football_EMOJI"]))
            if message:
